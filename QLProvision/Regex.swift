...
public struct Regex {
    ...
    public func replacingMatches(in input: String, with template: String, count: Int? = nil) -> String {
        let isGroupNo1 = template == "\\1"
        var output = input
        let matches = self.matches(in: input)
        let rangedMatches = Array(matches[0..<min(matches.count, count ?? matches.count)])
        for match in rangedMatches.reversed() {
            if isGroupNo1 {
                guard let string = match.captures.first, let withString = string else { continue }
                output.replaceSubrange(match.updateRange(output), with: withString)
            } else {
                let replacement = match.string(applyingTemplate: template)
                output.replaceSubrange(match.updateRange(output), with: replacement)
            }
        }
...
extension Regex {
    ...
    public class Match: CustomStringConvertible {
        ...
        public func updateRange(_ baseString:String) -> Range<String.Index> {
            return Range(self.result.range, in: baseString)!
        }
